...
export class FFQFoodNutrients {
  ...
  public static foodItemToResponse(
    fooditem: FFQFoodNutrients
  ): FFQFoodNutrientsResponse {
    const foodItemResponse = new FFQFoodItemResponse(
      fooditem.foodItem.name,
      fooditem.foodItem.id,
      fooditem.foodItem.itemPosition
    );
    ...
    for (var index in fooditem.nutrientList) {
      fooditem.nutrientList[index].nutrientListID =
        foodItemResponse.foodTypes[index].nutrientListID;
    }

    const foodNutrientsResponse = new FFQFoodNutrientsResponse(
      foodItemResponse,
      fooditem.nutrientList
    );
    return foodNutrientsResponse;
  }
  public static foodItemFromResponse(
    response: FFQFoodNutrientsResponse
  ): FFQFoodNutrients {
    console.log(response);
    console.log("here");

    const fooditem = new FFQFoodItem(
      response.foodItem.name,
      response.foodItem.itemPosition
    );
    ...
    if (response.foodItem.servingsList != null) {
      ...
      for (var serving of response.foodItem.servingsList) {
        ...
        if (index != 0) servingsString += serving["servingName"] + ", ";
        else servingsString += serving["servingName"];
      }
