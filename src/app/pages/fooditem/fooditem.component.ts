...
export class FooditemComponent implements OnInit {
  ...
  nutrientsMap: Map<string, FFQNutrientlist> = new Map<
    string,
    FFQNutrientlist
  >();
  ...
  ngOnInit() {
    ...
    if (FoodItemObjectId == "new") {
      ...
      this.ffqnutrientlist.push(
        new FFQNutrientlist("", new nutrientMap("", ""))
      );
      ...
      this.foodNutrients = new FFQFoodNutrients(
        this.ffqfoditem,
        this.ffqnutrientlist
      );
    ...
    this.foodService.getFoodbyName(name).subscribe((data) => {
      ...
      for (let i of data.foodItem.foodTypes) {
        this.nutrientsService
          .getNutrientsById(i.nutrientListID)
          .subscribe((nutrientList) => {
  ...
  private addFoodNutrients(form: NgForm) {
    ...
    this.foodService
      .addFoodNutrients(
        FFQFoodNutrients.foodItemToResponse(this.foodNutrientsItem[0])
      )
      .subscribe(
        (data) => {
